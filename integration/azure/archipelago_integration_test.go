...
func TestAzureArchipelagoLeaderFailover(t *testing.T) {
	...
	defer cancel()
	ensureAzureStoreReady(t, ctx, cfgA)
	ensureAzureStoreReady(t, ctx, cfgB)
	...
	tcA1 := startAzureArchipelagoNode(t, cfgA, addrA1, scheme, leaseTTL, fanoutGate)
	tcA2 := startAzureArchipelagoNode(t, cfgA, addrA2, scheme, leaseTTL, fanoutGate)
	tcB1 := startAzureArchipelagoNode(t, cfgB, addrB1, scheme, leaseTTL, fanoutGate)
	tcB2 := startAzureArchipelagoNode(t, cfgB, addrB2, scheme, leaseTTL, fanoutGate)
	...
	restartSpecs := []archipelagotest.RestartSpec{
		{
			...
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startAzureArchipelagoNode(tb, cfgA, addrA2, scheme, leaseTTL, fanoutGate, credsA2)
			},
		},
		{
			Index: 3,
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startAzureArchipelagoNode(tb, cfgB, addrB2, scheme, leaseTTL, fanoutGate, credsB2)
			},
		},
	}
	restartMap := map[int]archipelagotest.RestartSpec{
		0: {
			...
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startAzureArchipelagoNode(tb, cfgA, addrA1, scheme, leaseTTL, fanoutGate, credsA1)
			},
		},
		1: {
			Index: 1,
		...
		2: {
			...
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startAzureArchipelagoNode(tb, cfgB, addrB1, scheme, leaseTTL, fanoutGate, credsB1)
			},
		},
		3: {
			Index: 3,
...
func startAzureArchipelagoNode(tb testing.TB, base lockd.Config, addr, scheme string, leaseTTL time.Duration, gate *archipelagotest.FanoutGate, creds ...lockd.TestMTLSCredentials) *lockd.TestServer {
	tb.Helper()
	cfg := base
	cfg.Listen = addr
	cfg.ListenProto = "tcp"
	...
	return startAzureTestServer(tb, cfg, opts...)
}
