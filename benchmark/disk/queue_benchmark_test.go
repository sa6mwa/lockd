...
const diskQueuePayloadSize = 256

func BenchmarkDiskQueueAckLease(b *testing.B) {
	root := prepareDiskRoot(b, "")
	cfg := buildDiskConfig(b, root, 0)
	applyQueueBenchConfig(&cfg)
	client := startDiskBenchServer(b, cfg)

	payload := bytes.Repeat([]byte("x"), diskQueuePayloadSize)
