...
func TestStoreWriteSegmentDoesNotWarmCache(t *testing.T) {
	ctx := context.Background()
	const namespace = namespaces.Default
	backend := &countingGetObjectBackend{
		Backend: memory.New(),
	...
	store := NewStore(backend, nil)
	store.cache = newSegmentCache(4)

	seg := NewSegment("seg-write-no-warm", time.Unix(1_700_000_045, 0))
	seg.Fields["/status"] = FieldBlock{Postings: map[string][]string{
		"open": {"doc-1"},
	}}
	if _, _, err := store.WriteSegment(ctx, namespace, seg); err != nil {
		t.Fatalf("write segment: %v", err)
	}
	if got := segmentCacheSize(store.cache); got != 0 {
		t.Fatalf("expected write path to keep cache empty, got %d entries", got)
	}

	if _, err := store.LoadSegment(ctx, namespace, seg.ID); err != nil {
		t.Fatalf("load segment first: %v", err)
	}
	if got := backend.count(segmentObject(seg.ID)); got != 1 {
		t.Fatalf("expected one backend read after first load, got %d", got)
	}
	if got := segmentCacheSize(store.cache); got != 1 {
		t.Fatalf("expected first load to populate cache with one entry, got %d", got)
	}

	if _, err := store.LoadSegment(ctx, namespace, seg.ID); err != nil {
		t.Fatalf("load segment second: %v", err)
	}
	if got := backend.count(segmentObject(seg.ID)); got != 1 {
		t.Fatalf("expected second load to hit cache, backend reads=%d", got)
	}
}
...
func segmentCacheSize(c *segmentCache) int {
	if c == nil {
		return 0
	}
	c.mu.Lock()
