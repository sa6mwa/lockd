...
func (s *server) transferURL(capabilityID string) string {
	...
	u := *s.baseURL
	transferRoot := s.mcpServePath
	if strings.TrimSpace(transferRoot) == "" {
		transferRoot = scopedHTTPPath(cleanHTTPPath(s.baseURL.Path), s.mcpHTTPPath)
	}
	u.Path = path.Join(transferRoot, "transfer", strings.TrimSpace(capabilityID))
	u.RawQuery = ""
	u.Fragment = ""
	u.RawFragment = ""
	return u.String()
