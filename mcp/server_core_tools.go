...
func (s *server) handleLockAcquireTool(ctx context.Context, req *mcpsdk.CallToolRequest, input lockAcquireToolInput) (*mcpsdk.CallToolResult, lockAcquireToolOutput, error) {
	...
	lease, err := s.upstream.Acquire(ctx, api.AcquireRequest{
		Namespace:   s.resolveNamespaceForCall(input.Namespace, req),
		Key:         key,
		TTLSeconds:  ttl,
		Owner:       owner,
		BlockSecs:   block,
...
func (s *server) handleLockKeepAliveTool(ctx context.Context, req *mcpsdk.CallToolRequest, input lockKeepAliveToolInput) (*mcpsdk.CallToolResult, lockKeepAliveToolOutput, error) {
...
func (s *server) handleLockReleaseTool(ctx context.Context, req *mcpsdk.CallToolRequest, input lockReleaseToolInput) (*mcpsdk.CallToolResult, lockReleaseToolOutput, error) {
	if strings.TrimSpace(input.Key) == "" || strings.TrimSpace(input.LeaseID) == "" {
...
func (s *server) handleStatePutTool(ctx context.Context, req *mcpsdk.CallToolRequest, input statePutToolInput) (*mcpsdk.CallToolResult, statePutToolOutput, error) {
	key := strings.TrimSpace(input.Key)
	if key == "" {
		return nil, statePutToolOutput{}, fmt.Errorf("key is required")
	}
	if strings.TrimSpace(input.PayloadBase64) != "" && input.PayloadText != "" {
		return nil, statePutToolOutput{}, fmt.Errorf("payload_text and payload_base64 are mutually exclusive")
	}
	var payload []byte
	if strings.TrimSpace(input.PayloadBase64) != "" {
		...
		if err != nil {
			return nil, statePutToolOutput{}, fmt.Errorf("decode payload_base64: %w", err)
		}
		payload = decoded
	...
	if err := validateInlinePayloadBytes(int64(len(payload)), s.cfg.InlineMaxBytes, toolStatePut, toolStateWriteStreamBegin); err != nil {
	...
	updateResp, err := s.upstream.Update(ctx, key, lease.LeaseID, bytes.NewReader(payload), lockdclient.UpdateOptions{
		Namespace:    lease.Namespace,
		TxnID:        strings.TrimSpace(lease.TxnID),
	...
	if err != nil {
		if _, releaseErr := s.releaseFastPathLease(ctx, lease.Namespace, key, lease, true); releaseErr != nil {
			return nil, statePutToolOutput{}, fmt.Errorf("state.put failed: %w (rollback release failed: %v)", err, releaseErr)
		}
		return nil, statePutToolOutput{}, err
	}

	releaseResp, err := s.releaseFastPathLease(ctx, lease.Namespace, key, lease, false)
	if err != nil {
		return nil, statePutToolOutput{}, err
	}
	return nil, statePutToolOutput{
		Namespace:     lease.Namespace,
		Key:           key,
		TxnID:         lease.TxnID,
		CorrelationID: lease.CorrelationID,
		Committed:     releaseResp.Released,
		NewVersion:    updateResp.NewVersion,
		NewStateETag:  updateResp.NewStateETag,
		Bytes:         updateResp.BytesWritten,
		QueryHidden:   updateResp.Metadata.QueryHidden,
	}, nil
}

func (s *server) handleStateUpdateTool(ctx context.Context, req *mcpsdk.CallToolRequest, input stateUpdateToolInput) (*mcpsdk.CallToolResult, stateUpdateToolOutput, error) {
	if strings.TrimSpace(input.Key) == "" || strings.TrimSpace(input.LeaseID) == "" {
...
func (s *server) handleStateStreamTool(ctx context.Context, req *mcpsdk.CallToolRequest, input stateStreamToolInput) (*mcpsdk.CallToolResult, stateStreamToolOutput, error) {
	...
	opts := []lockdclient.GetOption{
		lockdclient.WithGetNamespace(s.resolveNamespaceForCall(input.Namespace, req)),
	}
	if leaseID != "" {
...
func (s *server) handleStateMetadataTool(ctx context.Context, req *mcpsdk.CallToolRequest, input stateMetadataToolInput) (*mcpsdk.CallToolResult, stateMetadataToolOutput, error) {
	if strings.TrimSpace(input.Key) == "" || strings.TrimSpace(input.LeaseID) == "" {
...
func (s *server) handleStateDeleteTool(ctx context.Context, req *mcpsdk.CallToolRequest, input stateDeleteToolInput) (*mcpsdk.CallToolResult, stateDeleteToolOutput, error) {
	key := strings.TrimSpace(input.Key)
	if key == "" {
		return nil, stateDeleteToolOutput{}, fmt.Errorf("key is required")
	}
	...
	if err != nil {
		if _, releaseErr := s.releaseFastPathLease(ctx, lease.Namespace, key, lease, true); releaseErr != nil {
			return nil, stateDeleteToolOutput{}, fmt.Errorf("state.delete failed: %w (rollback release failed: %v)", err, releaseErr)
		}
		return nil, stateDeleteToolOutput{}, err
	}
	...
	return nil, stateDeleteToolOutput{
		Namespace:     lease.Namespace,
		Key:           key,
		TxnID:         lease.TxnID,
		CorrelationID: lease.CorrelationID,
		Committed:     releaseResp.Released,
		Removed:       removeResp.Removed,
		NewVersion:    removeResp.NewVersion,
	}, nil
}
...
func (s *server) handleStateRemoveTool(ctx context.Context, req *mcpsdk.CallToolRequest, input stateRemoveToolInput) (*mcpsdk.CallToolResult, stateRemoveToolOutput, error) {
	if strings.TrimSpace(input.Key) == "" || strings.TrimSpace(input.LeaseID) == "" {
...
func (s *server) releaseFastPathLease(ctx context.Context, namespace string, key string, lease *lockdclient.LeaseSession, rollback bool) (*api.ReleaseResponse, error) {
	if lease == nil {
		return nil, fmt.Errorf("lease is required")
	}
	return s.upstream.Release(ctx, api.ReleaseRequest{
		Namespace: namespace,
		Key:       key,
...
func (s *server) handleAttachmentPutTool(ctx context.Context, req *mcpsdk.CallToolRequest, input attachmentPutToolInput) (*mcpsdk.CallToolResult, attachmentPutToolOutput, error) {
	key := strings.TrimSpace(input.Key)
	leaseID := strings.TrimSpace(input.LeaseID)
...
func (s *server) handleAttachmentListTool(ctx context.Context, req *mcpsdk.CallToolRequest, input attachmentListToolInput) (*mcpsdk.CallToolResult, attachmentListToolOutput, error) {
...
func (s *server) handleAttachmentGetTool(ctx context.Context, req *mcpsdk.CallToolRequest, input attachmentGetToolInput) (*mcpsdk.CallToolResult, attachmentGetToolOutput, error) {
	...
	_, head, err := s.handleAttachmentHeadTool(ctx, req, headIn)
	if err != nil {
...
func (s *server) handleAttachmentHeadTool(ctx context.Context, req *mcpsdk.CallToolRequest, input attachmentHeadToolInput) (*mcpsdk.CallToolResult, attachmentHeadToolOutput, error) {
	key := strings.TrimSpace(input.Key)
	if key == "" {
...
func (s *server) handleAttachmentChecksumTool(ctx context.Context, req *mcpsdk.CallToolRequest, input attachmentChecksumToolInput) (*mcpsdk.CallToolResult, attachmentChecksumToolOutput, error) {
	_, head, err := s.handleAttachmentHeadTool(ctx, req, attachmentHeadToolInput(input))
	if err != nil {
...
func (s *server) handleAttachmentDeleteTool(ctx context.Context, req *mcpsdk.CallToolRequest, input attachmentDeleteToolInput) (*mcpsdk.CallToolResult, attachmentDeleteToolOutput, error) {
	if strings.TrimSpace(input.Key) == "" || strings.TrimSpace(input.LeaseID) == "" {
...
func (s *server) handleAttachmentDeleteAllTool(ctx context.Context, req *mcpsdk.CallToolRequest, input attachmentDeleteAllToolInput) (*mcpsdk.CallToolResult, attachmentDeleteAllToolOutput, error) {
	if strings.TrimSpace(input.Key) == "" || strings.TrimSpace(input.LeaseID) == "" {
...
func (s *server) handleNamespaceGetTool(ctx context.Context, req *mcpsdk.CallToolRequest, input namespaceGetToolInput) (*mcpsdk.CallToolResult, namespaceGetToolOutput, error) {
	result, err := s.upstream.GetNamespaceConfig(ctx, s.resolveNamespaceForCall(input.Namespace, req))
	if err != nil {
...
func (s *server) handleNamespaceUpdateTool(ctx context.Context, req *mcpsdk.CallToolRequest, input namespaceUpdateToolInput) (*mcpsdk.CallToolResult, namespaceUpdateToolOutput, error) {
	ns := s.resolveNamespaceForCall(input.Namespace, req)
...
func (s *server) handleIndexFlushTool(ctx context.Context, req *mcpsdk.CallToolRequest, input indexFlushToolInput) (*mcpsdk.CallToolResult, indexFlushToolOutput, error) {
	...
	resp, err := s.upstream.FlushIndex(ctx, s.resolveNamespaceForCall(input.Namespace, req), opts...)
	if err != nil {
