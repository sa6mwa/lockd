...
- [x] 2026-02-26: Query-doc hotpath logging churn slice landed (pending commit): removed per-operation `logger.With(...)` enrichment in storage backend `loggers(ctx)` for disk/s3/aws to avoid repeated logger cloning on every state read/write. `lockd-bench` disk failover (`workload=query-index`, `query-return=documents`, `ops=4000`, `concurrency=8`, `runs=3`, `warmup=0`, `qrf-disabled`) median improved from ~`6511.9 ops/s` to `7140.1 ops/s` (+9.65%); matching `query-scan` moved from ~`540.1 ops/s` to `677.0 ops/s` (+25.35%).
- [x] 2026-02-26: Query-doc prefetch default tuning landed (pending commit): changed `DefaultQueryDocPrefetch` from `8` to `1` (still fully configurable) based on benchmark A/B where `prefetch=1` outperformed `prefetch=8` for disk query-doc workloads. Follow-up `lockd-bench` disk failover (`workload=query-index`, `query-return=documents`, `ops=4000`, `concurrency=8`, `runs=3`, `warmup=0`, `qrf-disabled`) median improved from `7140.1 ops/s` to `7474.6 ops/s` (+4.68%); `query-scan` remained stable (`676.3 ops/s` vs `677.0 ops/s` prior).
- [x] 2026-02-26: Query-doc state-read descriptor fast-path landed (pending commit): index adapter now attempts `ReadState` without per-document context overlays and only falls back to descriptor/size context when backend returns a missing-descriptor error. This removes per-document context wrapping in the hot path while preserving compatibility fallback. `lockd-bench` disk failover (`workload=query-index`, `query-return=documents`, `ops=4000`, `concurrency=8`, `runs=3`, `warmup=0`, `qrf-disabled`) median improved from `7474.6 ops/s` to `7693.4 ops/s` (+2.93%); `query-scan` remained within run-to-run variance (`676.3 ops/s` -> `669.3 ops/s`).
- [x] 2026-02-26: Storage logging `ReadState` hotpath logger-clone reduction landed (pending commit): removed unconditional `logger.With("storage_op", ...)` in storage logging wrapper start path and removed per-call `verbose.With("namespace", ...)` in `ReadState`. `lockd-bench` disk failover (`workload=query-index`, `query-return=documents`, `ops=4000`, `concurrency=8`, `runs=3`, `warmup=0`, `qrf-disabled`) median improved from `7394.8 ops/s` to `7623.6 ops/s` (+3.09%); `query-scan` remained stable (`674.1 ops/s` -> `677.0 ops/s`).
- [x] 2026-02-26: Disk `ReadState` descriptor-copy reduction landed (pending commit): removed one redundant per-read descriptor clone before decrypt/material resolution while preserving `StateInfo.Descriptor` copy semantics for callers. `lockd-bench` disk failover (`workload=query-index`, `query-return=documents`, `ops=4000`, `concurrency=8`, `runs=3`, `warmup=0`, `qrf-disabled`) median improved from `7623.6 ops/s` to `7738.9 ops/s` (+1.51%); `query-scan` remained in the same variance band (`677.0 ops/s` -> `671.8 ops/s`).
- [x] 2026-02-26: Storage crypto material reconstruction cache landed (pending commit): added bounded in-memory cache for `MaterialFromDescriptor` keyed by `(state-object-context, descriptor)` with cache behavior tests. `lockd-bench` disk failover (`workload=query-index`, `query-return=documents`, `ops=4000`, `concurrency=8`, `runs=3`, `warmup=0`, `qrf-disabled`) median improved from `7738.9 ops/s` to `8509.0 ops/s` (+9.95%); `query-scan` improved from `671.8 ops/s` to `706.5 ops/s`.
- [x] 2026-02-26: Retry `ReadState` hotpath closure elimination landed (pending commit): inlined transient retry loop for `ReadState` to remove per-call closure capture and `withRetry` indirection on the hottest query-doc path while preserving retry/backoff semantics. Controlled A/B (`lockd-bench` disk failover, fixed namespace `perfq-profile`, `query-seed=false`, `query-flush=false`, `query-return=documents`, `ops=12000`, `runs=3`, `warmup=0`, `concurrency=8`) improved median from `28495.1 ops/s` to `37516.1 ops/s` (+31.66%).
- [x] 2026-02-26: Client query stream reader pooling landed (pending commit): switched `client.newQueryStream` from per-request `bufio.NewReader` allocation to pooled `bufio.Reader` reuse with reset-on-close. Controlled A/B (`lockd-bench` disk failover, fixed namespace `perfq-profile`, `query-seed=false`, `query-flush=false`, `query-return=documents`, `ops=12000`, `runs=3`, `warmup=0`, `concurrency=8`) improved median from `37352.7 ops/s` to `40762.1 ops/s` (+9.13%). Alloc profile (`ops=24000`, `runs=1`) dropped total alloc-space from `~732.6MB` to `~592.7MB`; `bufio.NewReaderSize` dropped from `125.98MB` to `19.07MB`.
- [x] 2026-02-26: Index adapter query-reader pooling landed (pending commit): switched `segmentReader.cloneForQuery` from per-query allocation to pooled immutable reader clones and added deterministic release in `Adapter.Query`/`Adapter.QueryDocuments`. Immediate A/B (`lockd-bench` disk failover, fixed namespace `perfq-profile`, `query-seed=false`, `query-flush=false`, `query-return=documents`, `ops=24000`, `runs=3`, `warmup=0`, `concurrency=8`) improved median from `48465.6 ops/s` to `49720.2 ops/s` (+2.59%).
- [x] 2026-02-26: Single-endpoint client fast path landed (pending commit): `Client.shuffledEndpoints` now returns the existing slice without copy when only one endpoint is configured (common bench/runtime case). Query-index throughput stayed in the same band (median `49720.2 ops/s` vs `49703.4 ops/s` across immediate runs), while query-path allocation churn dropped (`net/url.parseQuery` alloc objects `93846` -> `58097` in one-run `alloc_objects` profiles).
