//go:build integration && minio && otlp && external && !lq
...
func TestMinioOTLPExternalTraceEmission(t *testing.T) {
	...
	logFile := filepath.Join(logDir, fmt.Sprintf("minio-otlp-external-%s.log", time.Now().Format("20060102T150405.000000000")))
	...
	if f, err := os.OpenFile(logFile, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0o644); err == nil {
		fmt.Fprintf(f, "=== minio external trace test %s ===\n", time.Now().Format(time.RFC3339Nano))
		_ = f.Close()
	} else {
	...
	key := "minio-otlp-external-" + uuidv7.NewString()
	lease := acquireWithRetry(t, ctx, cli, key, "otlp-external-worker", 30, 5)
