...
func (s *server) handleQueueEnqueueTool(ctx context.Context, req *mcpsdk.CallToolRequest, input queueEnqueueToolInput) (*mcpsdk.CallToolResult, queueEnqueueToolOutput, error) {
...
func (s *server) handleQueueStatsTool(ctx context.Context, req *mcpsdk.CallToolRequest, input queueStatsToolInput) (*mcpsdk.CallToolResult, queueStatsToolOutput, error) {
	namespace := s.resolveNamespaceForCall(input.Namespace, req)
	queue := s.resolveQueue(input.Queue)
...
func (s *server) handleQueueWatchTool(ctx context.Context, req *mcpsdk.CallToolRequest, input queueWatchToolInput) (*mcpsdk.CallToolResult, queueWatchToolOutput, error) {
	namespace := s.resolveNamespaceForCall(input.Namespace, req)
	queue := s.resolveQueue(input.Queue)
...
func (s *server) handleQueueAckTool(ctx context.Context, req *mcpsdk.CallToolRequest, input queueAckToolInput) (*mcpsdk.CallToolResult, queueAckToolOutput, error) {
	ackReq := api.AckRequest{
		Namespace:         s.resolveNamespaceForCall(input.Namespace, req),
	...
	if ackReq.Queue == "" || ackReq.MessageID == "" || ackReq.LeaseID == "" || ackReq.MetaETag == "" {
		return nil, queueAckToolOutput{}, fmt.Errorf("queue, message_id, lease_id, and meta_etag are required")
	}
	resp, err := s.upstream.QueueAck(ctx, ackReq)
	if err != nil {
...
func (s *server) handleQueueNackTool(ctx context.Context, req *mcpsdk.CallToolRequest, input queueNackToolInput) (*mcpsdk.CallToolResult, queueNackToolOutput, error) {
	...
	if reason := strings.TrimSpace(input.Reason); reason != "" {
		nackReq.LastError = map[string]any{"detail": reason}
	}
	if nackReq.Queue == "" || nackReq.MessageID == "" || nackReq.LeaseID == "" || nackReq.MetaETag == "" {
		return nil, queueNackToolOutput{}, fmt.Errorf("queue, message_id, lease_id, and meta_etag are required")
	}
	resp, err := s.upstream.QueueNack(ctx, nackReq)
	if err != nil {
...
func (s *server) handleQueueDeferTool(ctx context.Context, req *mcpsdk.CallToolRequest, input queueDeferToolInput) (*mcpsdk.CallToolResult, queueDeferToolOutput, error) {
	deferReq := api.NackRequest{
		Namespace:         s.resolveNamespaceForCall(input.Namespace, req),
		Queue:             strings.TrimSpace(input.Queue),
		MessageID:         strings.TrimSpace(input.MessageID),
		LeaseID:           strings.TrimSpace(input.LeaseID),
	...
	if deferReq.Queue == "" || deferReq.MessageID == "" || deferReq.LeaseID == "" || deferReq.MetaETag == "" {
		return nil, queueDeferToolOutput{}, fmt.Errorf("queue, message_id, lease_id, and meta_etag are required")
	}
	resp, err := s.upstream.QueueNack(ctx, deferReq)
	if err != nil {
...
func (s *server) handleQueueExtendTool(ctx context.Context, req *mcpsdk.CallToolRequest, input queueExtendToolInput) (*mcpsdk.CallToolResult, queueExtendToolOutput, error) {
	extendReq := api.ExtendRequest{
		Namespace:         s.resolveNamespaceForCall(input.Namespace, req),
		Queue:             strings.TrimSpace(input.Queue),
		MessageID:         strings.TrimSpace(input.MessageID),
		LeaseID:           strings.TrimSpace(input.LeaseID),
	...
	if extendReq.Queue == "" || extendReq.MessageID == "" || extendReq.LeaseID == "" || extendReq.MetaETag == "" {
		return nil, queueExtendToolOutput{}, fmt.Errorf("queue, message_id, lease_id, and meta_etag are required")
	}
	resp, err := s.upstream.QueueExtend(ctx, extendReq)
	if err != nil {
