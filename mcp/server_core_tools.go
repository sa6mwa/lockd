...
type txnDecisionToolInput struct {
	TxnID             string               `json:"txn_id" jsonschema:"Transaction id"`
	Participants      []api.TxnParticipant `json:"participants,omitempty" jsonschema:"Optional XA participant list"`
	ExpiresAtUnix     int64                `json:"expires_at_unix,omitempty" jsonschema:"Optional transaction expiry timestamp"`
	TCTerm            uint64               `json:"tc_term,omitempty" jsonschema:"Optional TC leader term"`
	TargetBackendHash string               `json:"target_backend_hash,omitempty" jsonschema:"Optional RM target backend hash"`
}

type txnDecisionToolOutput struct {
	TxnID string `json:"txn_id"`
	State string `json:"state"`
}

func (s *server) handleTxnPrepareTool(ctx context.Context, _ *mcpsdk.CallToolRequest, input txnDecisionToolInput) (*mcpsdk.CallToolResult, txnDecisionToolOutput, error) {
	resp, err := s.upstream.TxnPrepare(ctx, api.TxnDecisionRequest{
		TxnID:             strings.TrimSpace(input.TxnID),
		Participants:      append([]api.TxnParticipant(nil), input.Participants...),
		ExpiresAtUnix:     input.ExpiresAtUnix,
		TCTerm:            input.TCTerm,
		TargetBackendHash: strings.TrimSpace(input.TargetBackendHash),
	})
	if err != nil {
		return nil, txnDecisionToolOutput{}, err
	}
	return nil, txnDecisionToolOutput{TxnID: resp.TxnID, State: resp.State}, nil
}

func (s *server) handleTxnCommitTool(ctx context.Context, _ *mcpsdk.CallToolRequest, input txnDecisionToolInput) (*mcpsdk.CallToolResult, txnDecisionToolOutput, error) {
	resp, err := s.upstream.TxnCommit(ctx, api.TxnDecisionRequest{
		TxnID:             strings.TrimSpace(input.TxnID),
		Participants:      append([]api.TxnParticipant(nil), input.Participants...),
		ExpiresAtUnix:     input.ExpiresAtUnix,
		TCTerm:            input.TCTerm,
...
func (s *server) handleTxnRollbackTool(ctx context.Context, _ *mcpsdk.CallToolRequest, input txnDecisionToolInput) (*mcpsdk.CallToolResult, txnDecisionToolOutput, error) {
	resp, err := s.upstream.TxnRollback(ctx, api.TxnDecisionRequest{
		TxnID:             strings.TrimSpace(input.TxnID),
		Participants:      append([]api.TxnParticipant(nil), input.Participants...),
		ExpiresAtUnix:     input.ExpiresAtUnix,
		TCTerm:            input.TCTerm,
...
type txnReplayToolInput struct {
	TxnID string `json:"txn_id" jsonschema:"Transaction id"`
}

type txnReplayToolOutput struct {
	TxnID string `json:"txn_id"`
	State string `json:"state"`
}

func (s *server) handleTxnReplayTool(ctx context.Context, _ *mcpsdk.CallToolRequest, input txnReplayToolInput) (*mcpsdk.CallToolResult, txnReplayToolOutput, error) {
	resp, err := s.upstream.TxnReplay(ctx, strings.TrimSpace(input.TxnID))
	if err != nil {
		return nil, txnReplayToolOutput{}, err
	}
	return nil, txnReplayToolOutput{TxnID: resp.TxnID, State: resp.State}, nil
}
