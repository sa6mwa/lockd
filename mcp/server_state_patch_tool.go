...
func (s *server) handleStatePatchTool(ctx context.Context, req *mcpsdk.CallToolRequest, input statePatchToolInput) (*mcpsdk.CallToolResult, statePatchToolOutput, error) {
	...
	currentBytes, err := s.loadStateForPatch(ctx, key, input, req)
	if err != nil {
		return nil, statePatchToolOutput{}, err
	}
...
func (s *server) loadStateForPatch(ctx context.Context, key string, input statePatchToolInput, req *mcpsdk.CallToolRequest) ([]byte, error) {
	namespace := s.resolveNamespaceForCall(input.Namespace, req)
	leaseID := strings.TrimSpace(input.LeaseID)
