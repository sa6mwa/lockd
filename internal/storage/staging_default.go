...
func (d *defaultStagingBackend) LoadStagedState(ctx context.Context, namespace, key, txnID string) (io.ReadCloser, *StateInfo, error) {
...
func (d *defaultStagingBackend) PromoteStagedState(ctx context.Context, namespace, key, txnID string, opts PromoteStagedOptions) (*PutStateResult, error) {
	rdr, info, err := d.backend.ReadState(ctx, namespace, d.stagingKey(key, txnID))
	if err != nil {
		return nil, err
	}
	defer rdr.Close()
	res, err := d.backend.WriteState(ctx, namespace, key, rdr, PutStateOptions{ExpectedETag: opts.ExpectedHeadETag, Descriptor: info.Descriptor})
	if err != nil {
		return nil, err
	}
