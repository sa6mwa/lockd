...
const (
	...
	toolStateDelete                  = "lockd.state.delete"
...
func buildToolDescriptions(cfg Config) map[string]string {
	...
	return map[string]string{
		toolLockAcquire: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key` is required. `namespace` defaults to caller default namespace (client override or server default %q). `owner` defaults to OAuth client id. Optional `txn_id` enlists the lease in XA flow.", namespace)),
		...
		toolLockKeepAlive: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key`, `lease_id`, and `ttl_seconds > 0` are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional `txn_id` keeps XA context consistent.", namespace)),
		...
		toolLockRelease: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key` and `lease_id` are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional `txn_id` and `rollback=true` control XA/rollback behavior.", namespace)),
		...
		toolGet: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`key` is required. `namespace` defaults to caller default namespace (client override or server default %q). `public` defaults to true. If `public=false`, `lease_id` is required. `payload_mode` supports `auto` (default), `inline`, `stream`, or `none`; `auto` resolves to inline when payload bytes <= `lockd.hint.inline_max_payload_bytes`, otherwise stream.", namespace),
		...
		toolDescribe: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`key` is required. `namespace` defaults to caller default namespace (client override or server default %q) when omitted.", namespace),
		...
		toolQuery: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`query` expression is optional; empty string (`\"\"`) matches all keys. `namespace` defaults to caller default namespace (client override or server default %q). Optional `limit`, `cursor`, `engine`, `refresh`, and `fields` refine execution. Query output is always keys. Common patterns: `in{field=/tags,any=planning|finance}`, `icontains{field=/...,value=contract}`, `contains{f=/summary,a=\"renewal|master service agreement\"}`, `icontains{f=/...,a=\"contract|key phrase 2\"}`, `date{field=/updated_at,since=yesterday}`, `date{field=/updated_at,after=2025-01-01,before=2025-02-01}`, `range{field=/updated_at,gte=2025-01-01T00:00:00Z,lt=2025-02-01T00:00:00Z}`, and `/updated_at>=2025-01-01T00:00:00Z,/updated_at<2025-02-01T00:00:00Z`. For `contains`/`icontains`, use either `value=` or `any=` (not both). Relative macros (`now`, `today`, `yesterday`) are supported only in `date{...,since=...}`. Attachment payloads are blobs and are not queryable/indexed by LQL selectors.", namespace),
		...
		toolQueryStream: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("Active MCP session is required. `query` expression is optional; empty string (`\"\"`) matches all documents. `namespace` defaults to caller default namespace (client override or server default %q). Optional `limit`, `cursor`, `engine`, `refresh`, and `fields` refine execution. Recommended retrieval patterns mirror `lockd.query`: tags (`in{field=/tags,any=...}`), keyword sets (`contains`/`icontains` with `any=`), and temporal windows (`date{...}` or datetime `range`/shorthand bounds).", namespace),
		...
		toolStatePut: formatToolDescription(toolContract{
			Top: []string{
				bootstrapOverviewLine,
				leaseFinalizeLine,
			},
			Purpose:  "Fast-path JSON write: acquire -> update -> release in one tool call.",
			UseWhen:  "You want efficient document-store writes without manual lease choreography. This is the recommended insert/upsert path when lockd is used as a document/memory store rather than explicit coordination.",
			Requires: withAtomicity(fmt.Sprintf("`key` is required. `namespace` defaults to caller default namespace (client override or server default %q). Optional `owner`, `ttl_seconds`, and `block_seconds` tune lock behavior. Provide at most one of `payload_text` or `payload_base64`; when both are omitted payload defaults to `{}`. Decoded payload must be <= %d bytes. `query_hidden` defaults to false (query-visible) when omitted.", namespace, inlineMax)),
			Effects:  "Acquires lease, writes state with optional CAS/fencing guards (`if_etag`, `if_version`, `fencing_token`), then releases lease to commit in the same call. Returns update metadata.",
			Retry:    "If outcome is unknown, verify state/version before replay. Use `if_not_exists`, CAS, and fencing guards for deterministic retries.",
			Next:     nextWithBootstrapAndPayload("call `lockd.state.write_stream.begin` for large payload uploads.", "use `lockd.query`/`lockd.get` to verify written state or continue workflow."),
		}),
		toolStateUpdate: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key` and `lease_id` are required. `namespace` defaults to caller default namespace (client override or server default %q). Provide at most one of `payload_text` or `payload_base64`; when both are omitted payload defaults to `{}`. Decoded payload must be <= %d bytes.", namespace, inlineMax)),
		...
		toolStateMutate: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key`, `lease_id`, and at least one `mutations` expression are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional CAS/fencing fields (`if_etag`, `if_version`, `fencing_token`) are supported.", namespace)),
		...
		toolStatePatch: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key` and `lease_id` are required. `namespace` defaults to caller default namespace (client override or server default %q). Provide exactly one of `patch_text` or `patch_base64` and keep patch/result within inline limits; use `state.write_stream` for large documents.", namespace)),
		...
		toolStateWriteStreamBegin: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("Active MCP session is required. `key` and `lease_id` are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional CAS/fencing fields mirror `lockd.state.update`.", namespace),
		...
		toolStateStream: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("Active MCP session is required. `key` is required. `namespace` defaults to caller default namespace (client override or server default %q). `public` defaults to true; set `public=false` with `lease_id` for lease-bound reads.", namespace),
		...
		toolStateMetadata: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key`, `lease_id`, and `query_hidden` are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional CAS/fencing fields protect against races.", namespace)),
		...
		toolStateDelete: formatToolDescription(toolContract{
			Top: []string{
				bootstrapOverviewLine,
				leaseFinalizeLine,
			},
			Purpose:  "Fast-path delete: acquire -> remove -> release in one tool call.",
			UseWhen:  "You need efficient state deletion without manual lock lifecycle steps. This is the recommended delete path for document-store style workflows.",
			Requires: withAtomicity(fmt.Sprintf("`key` is required. `namespace` defaults to caller default namespace (client override or server default %q). Optional `owner`, `ttl_seconds`, and `block_seconds` tune lock behavior. Optional `if_etag`, `if_version`, and `fencing_token` protect against races.", namespace)),
			Effects:  "Acquires lease, removes state, then releases lease to commit delete in the same call. Returns removal and version metadata.",
			Retry:    "If outcome is unknown, verify key existence/version before replay. Prefer CAS/fencing guards for deterministic retries.",
			Next:     nextWithBootstrap("use `lockd.describe`/`lockd.query` to confirm deletion or continue workflow."),
		}),
		toolStateRemove: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key` and `lease_id` are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional `txn_id` and CAS/fencing fields support XA/concurrency control.", namespace)),
		...
		toolAttachmentsPut: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key`, `lease_id`, and `name` are required. `namespace` defaults to caller default namespace (client override or server default %q). Provide at most one of `payload_text` or `payload_base64`; decoded payload must be <= %d bytes. `mode` supports `create` (default), `upsert`, `replace`.", namespace, inlineMax)),
		...
		toolAttachmentsWriteStreamBegin: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("Active MCP session is required. `key`, `lease_id`, and `name` are required. `namespace` defaults to caller default namespace (client override or server default %q). `mode` supports `create` (default), `upsert`, `replace`.", namespace),
		...
		toolAttachmentsList: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`key` is required. `namespace` defaults to caller default namespace (client override or server default %q). `public` defaults to true. If `public=false`, `lease_id` is required. If `public=true`, `lease_id` must be omitted.", namespace),
		...
		toolAttachmentsHead: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`key` plus either `id` or `name` is required. `namespace` defaults to caller default namespace (client override or server default %q). `public` defaults to true. If `public=false`, `lease_id` is required. If `public=true`, `lease_id` must be omitted.", namespace),
		...
		toolAttachmentsChecksum: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`key` plus either `id` or `name` is required. `namespace` defaults to caller default namespace (client override or server default %q). `public` defaults to true. If `public=false`, `lease_id` is required.", namespace),
		...
		toolAttachmentsGet: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`key` plus either `id` or `name` is required. `namespace` defaults to caller default namespace (client override or server default %q). `public` defaults to true; if `public=false`, `lease_id` is required and if `public=true`, `lease_id` must be omitted. `payload_mode` supports `auto` (default), `inline`, `stream`, or `none`; `auto` resolves to inline when payload bytes <= `lockd.hint.inline_max_payload_bytes`, otherwise stream.", namespace),
		...
		toolAttachmentsStream: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("Active MCP session is required. `key` plus either `id` or `name` is required. `namespace` defaults to caller default namespace (client override or server default %q). `public` defaults to true; if `public=false`, `lease_id` is required and if `public=true`, `lease_id` must be omitted.", namespace),
		...
		toolAttachmentsDelete: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key` and `lease_id` are required, plus either `id` or `name`. `namespace` defaults to caller default namespace (client override or server default %q). Optional `txn_id` enables XA staging.", namespace)),
		...
		toolAttachmentsDeleteAll: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`key` and `lease_id` are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional `txn_id` allows transactional staging.", namespace)),
		...
		toolNamespaceGet: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`namespace` defaults to caller default namespace (client override or server default %q) when omitted.", namespace),
		...
		toolNamespaceUpdate: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`preferred_engine` and `fallback_engine` are required. `namespace` defaults to caller default namespace (client override or server default %q). Optional `if_match` enables CAS-safe updates.", namespace),
			...
			Next:     nextWithBootstrap("rerun `lockd.query`/`lockd.query.stream` to validate behavior."),
		}),
		toolIndexFlush: formatToolDescription(toolContract{
			Purpose:  "Trigger administrative search index flush for a namespace.",
			UseWhen:  "Operator-directed diagnostics or maintenance workflows that explicitly require flush control.",
			Requires: fmt.Sprintf("`namespace` defaults to caller default namespace (client override or server default %q). `mode` may be `async` (default) or `wait`.", namespace),
			...
			Next:     "Return to the calling administrative workflow.",
		}),
		...
		toolHelp: formatToolDescription(toolContract{
			Purpose:  "Return curated lockd MCP workflows, invariants, and documentation resource URIs, including document-store fastpaths and comprehensive LQL guidance.",
			UseWhen:  "Start of session or when uncertain about correct operation sequence.",
			Requires: "No required fields. Optional `topic` narrows guidance to overview/locks/messaging/sync/lql (including date/range temporal selectors, inferred shorthand datetime operators, and contains/icontains any-forms).",
			Effects:  "Returns guidance only; no lockd state mutation occurs.",
			Retry:    "Safe to retry.",
			Next:     "Follow `next_calls` in the response, typically beginning with query tools and state.put/state.delete fastpaths for document-store workflows.",
		}),
		toolQueueEnqueue: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`queue` defaults to %q. `namespace` defaults to caller default namespace (client override or server default %q). Provide at most one of `payload_text` or `payload_base64`; decoded payload must be <= %d bytes.", queue, namespace, inlineMax),
		...
		toolQueueWriteStreamBegin: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("Active MCP session is required. `queue` defaults to %q and `namespace` defaults to caller default namespace (client override or server default %q). Delay/visibility/ttl/attributes/content_type options match `lockd.queue.enqueue`.", queue, namespace),
		...
		toolQueueDequeue: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`queue` defaults to %q. `namespace` defaults to caller default namespace (client override or server default %q). `owner` defaults to OAuth client id. Optional `stateful`, `visibility_seconds`, `cursor`, and `txn_id` tune dequeue behavior. `payload_mode` supports `auto` (default), `inline`, `stream`, `none`; `state_mode` (when `stateful=true`) supports the same values. For both modes, `auto` resolves inline when payload bytes <= `lockd.hint.inline_max_payload_bytes`, otherwise stream.", queue, namespace)),
		...
		toolQueueStats: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`queue` defaults to %q and `namespace` defaults to caller default namespace (client override or server default %q).", queue, namespace),
		...
		toolQueueWatch: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("`queue` defaults to %q and `namespace` defaults to caller default namespace (client override or server default %q). Optional `duration_seconds` bounds wait time (default 30). Optional `max_events` bounds result size (default 1).", queue, namespace),
		...
		toolQueueAck: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`queue`, `message_id`, `lease_id`, and `meta_etag` are required. `namespace` defaults to caller default namespace (client override or server default %q). Include `fencing_token` and any `state_*` fields exactly as returned by `lockd.queue.dequeue` for deterministic queue/state coordination.", namespace)),
		...
		toolQueueNack: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`queue`, `message_id`, `lease_id`, and `meta_etag` are required. `namespace` defaults to caller default namespace (client override or server default %q). Include `fencing_token` and any `state_*` fields from dequeue. Optional `delay_seconds` and `reason` enrich retry behavior.", namespace)),
		...
		toolQueueDefer: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`queue`, `message_id`, `lease_id`, and `meta_etag` are required. `namespace` defaults to caller default namespace (client override or server default %q). Include `fencing_token` and any `state_*` fields from dequeue. Optional `delay_seconds` postpones visibility.", namespace)),
		...
		toolQueueExtend: formatToolDescription(toolContract{
			...
			Requires: withAtomicity(fmt.Sprintf("`queue`, `message_id`, `lease_id`, and `meta_etag` are required. `namespace` defaults to caller default namespace (client override or server default %q). Include `fencing_token` and optional state lease fields from dequeue for stateful workflows.", namespace)),
		...
		toolQueueSubscribe: formatToolDescription(toolContract{
			...
			Requires: fmt.Sprintf("Active MCP session is required. `queue` defaults to %q and `namespace` defaults to caller default namespace (client override or server default %q).", queue, namespace),
