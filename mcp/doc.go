...
//   - Hosts local OAuth 2.1 endpoints for both confidential m2m clients and
//     authorization-code + PKCE interactive clients
//   - Enforces bearer-token auth by default when TLS is enabled
//   - Scopes OAuth/discovery routes under BaseURL path, and MCP/transfer routes
//     under BaseURL path + MCPPath
...
// configuration are persisted under the lockd config directory. Issued OAuth
// access tokens are persisted in an encrypted token store so active tokens
// remain valid across MCP restarts until expiry or revocation.
//
// If OAuth state is missing and TLS is enabled, NewServer returns an error
// instructing the operator to initialize MCP OAuth state first (for example via
// `lockd mcp issuer set <url>` or `lockd mcp client add`).
//
...
//     `OAuthTokenStorePath`, `InlineMaxBytes`, `DefaultNamespace`,
//     `AgentBusQueue`)
//
