...
func (s *Service) Update(ctx context.Context, cmd UpdateCommand) (*UpdateResult, error) {
	...
	for {
		...
		if s.indexManager != nil && result.BytesWritten > 0 && result.BytesWritten <= maxStagedIndexDocBytes {
			if stagedReader, readerErr := spool.Reader(); readerErr == nil {
				if doc, docErr := buildDocumentFromJSON(keyComponent, stagedReader); docErr == nil {
					s.cacheStagedIndexDoc(namespace, keyComponent, cmd.TxnID, result.NewETag, result.BytesWritten, doc)
				}
			}
		}
		return &UpdateResult{
