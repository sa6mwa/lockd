...
func (m *subscriptionManager) run(ctx context.Context, sub *queueSubscription) {
	...
	if err != nil && !isBenignSubscriptionStop(err) {
...
func isBenignSubscriptionStop(err error) bool {
	if err == nil {
		return false
	}
	...
	return strings.Contains(msg, "undelivered message: stream not connected or already closed")
}
