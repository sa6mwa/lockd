...
func startLockdServer(ctx context.Context, cfg envConfig) (*lockdServerHandle, error) {
	...
	handle, err := lockd.StartServer(ctx, serverCfg, lockd.WithLogger(pslog.NewStructured(context.Background(), io.Discard)))
	if err != nil {
		return nil, err
	}
	srv := handle.Server
...
func exerciseLockd(ctx context.Context, cfg envConfig, baseURL, key string) (string, string, error) {
	cli, err := lockdclient.New(baseURL,
		...
		lockdclient.WithLogger(pslog.NewStructured(context.Background(), io.Discard)),
	)
	if err != nil {
