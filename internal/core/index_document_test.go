...
func TestBuildDocumentFromJSONIndexesAllObjectFields(t *testing.T) {
	doc, err := buildDocumentFromJSON("k1", strings.NewReader(`{"a":"one","b":"two","nested":{"c":"three"}}`))
	if err != nil {
		t.Fatalf("buildDocumentFromJSON: %v", err)
	}
	if raw := doc.Fields["/a"]; len(raw) == 0 || raw[0] != "one" {
		t.Fatalf("unexpected /a terms: %+v", raw)
	}
	if raw := doc.Fields["/b"]; len(raw) == 0 || raw[0] != "two" {
		t.Fatalf("unexpected /b terms: %+v", raw)
	}
	if raw := doc.Fields["/nested/c"]; len(raw) == 0 || raw[0] != "three" {
		t.Fatalf("unexpected /nested/c terms: %+v", raw)
	}
}
