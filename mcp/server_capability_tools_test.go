...
func TestHandleQueryToolEmptyQueryMatchesAll(t *testing.T) {
	t.Parallel()

	s, cli := newToolTestServer(t)
	ctx := context.Background()
	key := fmt.Sprintf("mcp-query-empty-%d", time.Now().UnixNano())
	lease, err := cli.Acquire(ctx, api.AcquireRequest{
		Namespace:  "mcp",
		Key:        key,
		TTLSeconds: 30,
	...
	if _, err := cli.UpdateBytes(ctx, key, lease.LeaseID, []byte(`{"kind":"mcp-query-empty","ok":true}`), lockdclient.UpdateOptions{
		Namespace: "mcp",
	}); err != nil {
		t.Fatalf("update: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("query tool with empty query: %v", err)
	}
	if !slices.Contains(out.Keys, key) {
		t.Fatalf("expected empty query to include key %q, got %v", key, out.Keys)
	}
}
...
func TestHandleQueryStreamToolEmptyQueryMatchesAll(t *testing.T) {
	t.Parallel()

	s, cli := newToolTestServer(t)
	ctx := context.Background()
	key := fmt.Sprintf("mcp-query-stream-empty-%d", time.Now().UnixNano())
	lease, err := cli.Acquire(ctx, api.AcquireRequest{
		Namespace:  "mcp",
		Key:        key,
		TTLSeconds: 30,
	...
	if _, err := cli.UpdateBytes(ctx, key, lease.LeaseID, []byte(`{"kind":"mcp-query-stream-empty","ok":true}`), lockdclient.UpdateOptions{
		Namespace: "mcp",
	}); err != nil {
		t.Fatalf("update: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("query stream tool with empty query: %v", err)
	}
	ndjson := string(mustDownloadTransfer(t, s, out.DownloadURL))
	if !strings.Contains(ndjson, `"key":"`+key+`"`) {
		t.Fatalf("expected empty query stream NDJSON to include key %q: %s", key, ndjson)
	}
}
...
func TestHandleStatePutToolFastPathCommitsAndIsQueryVisibleByDefault(t *testing.T) {
	t.Parallel()

	s, _ := newToolTestServer(t)
	ctx := context.Background()
	key := fmt.Sprintf("mcp-state-put-fastpath-%d", time.Now().UnixNano())

	_, out, err := s.handleStatePutTool(ctx, nil, statePutToolInput{
		Namespace:   "mcp",
		Key:         key,
		PayloadText: `{"tags":["fastpath","memory"],"kind":"state-put-fastpath"}`,
	})
	if err != nil {
		t.Fatalf("state.put: %v", err)
	}
	if !out.Committed {
		t.Fatalf("expected committed=true, got false")
	}
	if out.NewVersion == 0 {
		t.Fatalf("expected non-zero new version")
	}

	_, queryOut, err := s.handleQueryTool(ctx, nil, queryToolInput{
		Namespace: "mcp",
		Query:     "",
		Engine:    "scan",
	})
	...
	if !slices.Contains(queryOut.Keys, key) {
		t.Fatalf("expected key %q in query results, got %v", key, queryOut.Keys)
	}
}

func TestHandleStateDeleteToolFastPathRemovesState(t *testing.T) {
	t.Parallel()

	s, _ := newToolTestServer(t)
	ctx := context.Background()
	key := fmt.Sprintf("mcp-state-delete-fastpath-%d", time.Now().UnixNano())

	if _, _, err := s.handleStatePutTool(ctx, nil, statePutToolInput{
		Namespace:   "mcp",
		Key:         key,
		PayloadText: `{"kind":"state-delete-fastpath","ok":true}`,
	}); err != nil {
		t.Fatalf("seed state.put: %v", err)
	}

	_, delOut, err := s.handleStateDeleteTool(ctx, nil, stateDeleteToolInput{
		Namespace: "mcp",
		Key:       key,
	})
	if err != nil {
	...
	_, getOut, err := s.handleGetTool(ctx, nil, getToolInput{
		Namespace:   "mcp",
		Key:         key,
		PayloadMode: "none",
	})
...
func TestHandleStatePutToolReleasesLeaseOnMutationFailure(t *testing.T) {
	t.Parallel()

	s, cli := newToolTestServer(t)
	ctx := context.Background()
	key := fmt.Sprintf("mcp-state-put-release-failure-%d", time.Now().UnixNano())

	if _, _, err := s.handleStatePutTool(ctx, nil, statePutToolInput{
		Namespace:   "mcp",
		Key:         key,
		PayloadText: `{"kind":"seed","v":1}`,
	}); err != nil {
		t.Fatalf("seed state.put: %v", err)
	}

	badVersion := int64(999999)
	if _, _, err := s.handleStatePutTool(ctx, nil, statePutToolInput{
		Namespace:   "mcp",
		Key:         key,
		IfVersion:   &badVersion,
		PayloadText: `{"kind":"seed","v":2}`,
	}); err == nil {
		t.Fatalf("expected state.put to fail with CAS mismatch")
	}
	...
	if err != nil {
		t.Fatalf("acquire after failed state.put: %v", err)
	}
	if _, err := cli.Release(ctx, api.ReleaseRequest{
		Namespace: "mcp",
		Key:       key,
		...
		t.Fatalf("release after failed state.put: %v", err)
	}
}

func TestHandleStateDeleteToolReleasesLeaseOnMutationFailure(t *testing.T) {
	t.Parallel()

	s, cli := newToolTestServer(t)
	ctx := context.Background()
	key := fmt.Sprintf("mcp-state-delete-release-failure-%d", time.Now().UnixNano())

	if _, _, err := s.handleStatePutTool(ctx, nil, statePutToolInput{
		Namespace:   "mcp",
		Key:         key,
		PayloadText: `{"kind":"seed-delete","v":1}`,
	}); err != nil {
		t.Fatalf("seed state.put: %v", err)
	}

	badVersion := int64(999999)
	if _, _, err := s.handleStateDeleteTool(ctx, nil, stateDeleteToolInput{
		Namespace: "mcp",
		Key:       key,
		IfVersion: &badVersion,
		IfETag:    "invalid-etag-precondition",
	}); err == nil {
		t.Fatalf("expected state.delete to fail with CAS mismatch")
	}
	...
	if err != nil {
		t.Fatalf("acquire after failed state.delete: %v", err)
	}
	if _, err := cli.Release(ctx, api.ReleaseRequest{
		Namespace: "mcp",
		Key:       key,
		...
		t.Fatalf("release after failed state.delete: %v", err)
	}
}
