...
- Local OAuth 2.1 provider enabled by default (all auth/discovery routes are scoped under `base-url` path)
...
- `GET <base-url-path>/authorize`
- `POST <base-url-path>/token`
- `POST <base-url-path>/register`
- `GET <base-url-path>/.well-known/oauth-authorization-server`
- `GET <base-url-path>/.well-known/openid-configuration`
- `GET <base-url-path>/.well-known/oauth-protected-resource[/(resource-suffix)]`

Built-in OAuth supports both machine-to-machine (`client_credentials`) and authorization code + PKCE (`authorization_code` + `refresh_token`) flows.
...
- OAuth access-token store path: `mcp.token-store` (default `~/.lockd/mcp-token-store.enc.json`)
- Access tokens are persisted encrypted at rest so valid tokens survive MCP restarts until expiry or revocation.
...
1. Initialize issuer and state (auto-creates MCP OAuth state on first run):

```bash
lockd mcp issuer set https://localhost:19341
```

2. Create a client:

```bash
lockd mcp client add
```

3. Print agent configuration values for that client:

```bash
lockd mcp client credentials <client_id> --format env
```
...
If OAuth state is missing and TLS is enabled, `lockd mcp` initializes state automatically using a safe default bootstrap client name (`default`) without printing bootstrap credentials.
...
- `--base-url`: externally reachable MCP base URL; OAuth/.well-known are rooted at this path and `mcp-path` is resolved beneath it (required)
...
- `--token-store`: encrypted OAuth access-token store path
- `--issuer`: global issuer override for first-run runtime state initialization (`lockd mcp` / `lockd mcp client add`); if empty, issuer defaults to `--base-url`
- `--mcp-path`: MCP streamable endpoint path relative to `--base-url` path (default `/`)
- `--oauth-resource-url`: protected resource identifier URL (default `<issuer> + <mcp-path>`)
...
OAuth metadata/discovery path behavior:

- OAuth discovery and auth endpoints are scoped under `base-url` path.
- MCP stream and transfer endpoints are scoped under `base-url path + mcp-path`.
- Example: `base-url=https://example.tld/mcp` and `mcp-path=/` serves discovery/auth under `/mcp/...`.
- Example: `base-url=https://mcp.example.tld` and `mcp-path=/subpath` serves discovery/auth under `/...` and MCP under `/subpath/...`.
- Recommended production deployment: dedicate an MCP hostname and keep `mcp-path=/` (example: `--base-url https://mcp.example.tld --mcp-path /`).

### `lockd mcp` admin surface

- `lockd mcp issuer get`
- `lockd mcp issuer set <url>`
...
- `lockd mcp client restore <client_id>`
- `lockd mcp client remove <client_id>`
- `lockd mcp client rotate-secret <client_id>`
...
- `mcp.token_store` -> `LOCKD_MCP_TOKEN_STORE`
...
- Admin read command error: `mcp oauth state missing`:
  - initialize state by running `lockd mcp`, `lockd mcp issuer set <url>`, or `lockd mcp client add`
