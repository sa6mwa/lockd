...
func TestAWSStartConsumerAutoAckRegression(t *testing.T) {
	queuetestutil.InstallWatchdog(t, "aws-start-consumer-auto-ack", 2*time.Minute)
	...
	queuetestutil.RunStartConsumerAutoAckRegression(t, ts.Client, queuetestutil.StartConsumerAutoAckRegressionOptions{
		Label:   "aws-start-consumer-auto-ack",
		Timeout: 90 * time.Second,
	})
}
