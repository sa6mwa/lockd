...
func TestMinioArchipelagoLeaderFailover(t *testing.T) {
	...
	tcA1 := startMinioArchipelagoNode(t, cfgA, addrA1, scheme, leaseTTL, fanoutGate)
	tcA2 := startMinioArchipelagoNode(t, cfgA, addrA2, scheme, leaseTTL, fanoutGate)
	tcB1 := startMinioArchipelagoNode(t, cfgB, addrB1, scheme, leaseTTL, fanoutGate)
	tcB2 := startMinioArchipelagoNode(t, cfgB, addrB2, scheme, leaseTTL, fanoutGate)
	...
	restartSpecs := []archipelagotest.RestartSpec{
		{
			...
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startMinioArchipelagoNode(tb, cfgA, addrA2, scheme, leaseTTL, fanoutGate, credsA2)
			},
		},
		{
			Index: 3,
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startMinioArchipelagoNode(tb, cfgB, addrB2, scheme, leaseTTL, fanoutGate, credsB2)
			},
		},
	}
	restartMap := map[int]archipelagotest.RestartSpec{
		0: {
			...
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startMinioArchipelagoNode(tb, cfgA, addrA1, scheme, leaseTTL, fanoutGate, credsA1)
			},
		},
		1: {
			Index: 1,
		...
		2: {
			...
			Start: func(tb testing.TB) *lockd.TestServer {
				tb.Helper()
				return startMinioArchipelagoNode(tb, cfgB, addrB1, scheme, leaseTTL, fanoutGate, credsB1)
			},
		},
		3: {
			Index: 3,
...
func startMinioArchipelagoNode(tb testing.TB, base lockd.Config, addr, scheme string, leaseTTL time.Duration, gate *archipelagotest.FanoutGate, creds ...lockd.TestMTLSCredentials) *lockd.TestServer {
	tb.Helper()
	cfg := base
	cfg.Listen = addr
	cfg.ListenProto = "tcp"
