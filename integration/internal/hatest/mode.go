...
func WaitForPassiveNode(t testing.TB, ts *lockd.TestServer, timeout time.Duration) {
	...
	if err != nil {
		t.Fatalf("wait for passive node client: %v", err)
	}
	defer cli.Close()
	...
	var lastErr error
	for time.Now().Before(deadline) {
		err := passiveNodeProbeClient(cli)
		if err == nil {
			return
		}
		lastErr = err
...
func passiveNodeProbe(ts *lockd.TestServer) error {
	...
	defer cli.Close()
	return passiveNodeProbeClient(cli)
}

func passiveNodeProbeClient(cli *lockdclient.Client) error {
	if cli == nil {
		return fmt.Errorf("require passive node: nil client")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	lease, err := cli.Acquire(ctx, api.AcquireRequest{
